...
mainDir="/Volumes/MAOS 4/LR-BcatNodalhethom/Appended/";

//saveDir="/Volumes/MAOS 2/Spinning Disk/E22-10R-fixed/E22-10R-Bcat-fixed";
saveDir=mainDir;
...
hyperstacks= 1;
cc=2;  //indicate how many channels it has, this helps a lot ! 
timepoints=36; // indicate how many timepoints the movie has, this helps tbh, I will automate later
...
saveName="FIJI-video-"+substring(filesname, 0,lastIndexOf(filesname, "-2"))+"-"+substring(subDir, 0, lastIndexOf(subDir,"/"))+".tif";
...
newslices=(slices/cc)/timepoints;
...
//comment next if they're hyperstacks:
if (hyperstacks<1) {
"---Transforming to Hyperstacks"
selectWindow(filename1);
run("Stack to Hyperstack...", "order=xyczt(default) channels=cc slices=newslices frames=timepoints display=Color");
selectWindow(filename2);
run("Stack to Hyperstack...", "order=xyczt(default) channels=cc slices=newslices frames=timepoints display=Color");
}
//comment before if they're hyperstacks


print("---First pair-stitching initiated");
print("---Please wait...");
run("Pairwise stitching", "first_image=[filename1] second_image=[filename2] fusion_method=[Linear Blending] fused_image=[filename1<->filename2] check_peaks=5 Ignore_zero_values_when_fusing compute_overlap x=0.0000 y=0.0000 z=0.0000 registration_channel_image_1=[Only channel 1] registration_channel_image_2=[Only channel 1] time-lapse_registration=[Apply registration of first time-point to all other time-points]");
...
//comment if they're hyperstacks
if (hyperstacks<1) {
"---Transforming to Hyperstack"
...
print("---Please wait...");
run("Pairwise stitching", "first_image=[filename1<->filename2] second_image=[filename3] fusion_method=[Linear Blending] fused_image=[allfused] check_peaks=5 Ignore_zero_values_when_fusing compute_overlap x=0.0000 y=0.0000 z=0.0000 registration_channel_image_1=[Only channel 1] registration_channel_image_2=[Only channel 1] time-lapse_registration=[Apply registration of first time-point to all other time-points]");
print("---Saving...");
